...
class CSVProcessor:
    ...
    def process_csv_content(cls, content: bytes, filename: str) -> Tuple[pd.DataFrame, Dict]:
        ...
        try:
            ...
            try:
                dialect = csv.Sniffer().sniff(content_str.splitlines()[0])
                sep = dialect.delimiter
            except Exception:
                sep = ","
            
            # Read CSV
            df = pd.read_csv(StringIO(content_str), sep=sep)
    ...
    def _clean_dataframe(cls, df: pd.DataFrame) -> pd.DataFrame:
        ...
        string_columns = ['kepoi_name', 'kepler_name', 'koi_disposition', 'koi_pdisposition']
