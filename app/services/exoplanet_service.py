...
class ExoplanetService:
    ...
    def get_candidates(
        ...
        if status_filter:
            query = query.filter(
                ExoplanetCandidate.analysis_status == status_filter)

        if verdict_filter:
            query = query.filter(
                ExoplanetCandidate.final_verdict == verdict_filter)
    ...
    def bulk_create_from_csv(
        ...
        try:
            for idx, row in df.iterrows():
                try:
                    ...
                    db_candidate = ExoplanetCandidate(
                        ...
                        koi_fpflag_ss=row_dict.get('koi_fpflag_ss'),
                        koi_fpflag_co=row_dict.get('koi_fpflag_co'),
                        koi_fpflag_nt=row_dict.get('koi_fpflag_nt'),
                        koi_fpflag_ec=row_dict.get('koi_fpflag_ec')
                    )
                    ...
                    logger.error(
                        f"Error processing row {idx}: {str(row_error)}")
            ...
            logger.info(
                f"Successfully created {len(candidates)} candidates from CSV")
            return candidates
